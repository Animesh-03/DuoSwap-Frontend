    ...
    return (
        <Modal className="flex items-center justify-center" open={open} onClose={() => setOpen(false)} BackdropProps={{style: {backgroundColor: "#b6eeacA0"}}}>
            <div className={clsx([css.root])}>
                ...
                <WalletCard src="/catalog.svg" width={32} height={32} alt="catalog" title="Catalog Wallet" onSuccess={() => setOpen(false)} />
                <WalletCard src="/metamask.svg" width={32} height={32} alt="catalog" title="Metamask Wallet" onSuccess={() => setOpen(false)} />
            </div>
        </Modal>
    )
}